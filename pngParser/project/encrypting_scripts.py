...
def encrypt_data_compressd(data, length):
    ...
    for i in range(0,len(data),size_of_block):
        ...
        block = cipher_text.to_bytes(int(2*n.bit_length()), 'big') # tworzony jest blok o długości n w bajtach tak aby była stała szerokość
...
def decrypt_data_CBC(data, private_key,chunk_size, vector,orginal_file_data_length):
    ...
    size_of_block = int(n.bit_length()/8)
    data = bytearray(data)
...
def decrypt_compressed(data, private_key,chunk_size,orginal_file_data_length):

    n = private_key[0]
    d = private_key[1]

    size_of_block = int(2*n.bit_length())
    data = bytearray(data)
